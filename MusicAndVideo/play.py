...
AMBILFOTO = [
    "https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
    "https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
    "https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
    "https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
    "https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
...
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("**ثواني بس ابحثلك عليها.**")
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
                    caption=f"""
⌔ ابسط ضيفتها للانتظار {pos}
⌔ الاسم: [{songname}]({link})
...
""",
                ...
                add_to_queue(chat_id, songname, dl, link, "الصوت", 0)
                await huehue.delete()
                # await m.reply_to_message.delete()
                await m.reply_photo(
                    photo="https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
...
""",
        ...
        if len(m.command) < 2:
            await m.reply("⌔ ابعت اسم اغنيه او ارجع لزر الاوامر لمعرفة استخدامي.")
        else:
            await m.delete()
            huehue = await m.reply("ثواني بس ابحث عليها.")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await huehue.edit("مش لاقي حاجه ابعت اسم المغني كامل كدا")
            else:
                songname = search[0]
                url = search[1]
...
async def vplay(client, m: Message):
    ...
    if replied:
        if replied.video or replied.document:
            await m.delete()
            huehue = await replied.reply("**ثواني بس ابحثلك عنها.**")
            dl = await replied.download()
            link = replied.link
            if len(m.command) < 2:
                Q = 720
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/1ad3ae6099853bd4f2a7f.jpg",
                    caption=f"""
**⌔ ابسط ضيفتهالك لقايمة الانتظار {pos}
⌔ الاسم : [{songname}]({link})
⌔ ايدي المحادثه : {chat_id}
⌔ طلب الكائن دا : {m.from_user.mention}**
""",
...
""",
        ...
        if len(m.command) < 2:
            await m.reply(
                "**⌔ اكتب اسم فيديو او ارجع لزر الاوامر لمعرفة استخدامي.**"
            )
        else:
            await m.delete()
            huehue = await m.reply("**اصبر ببحثلك عليها اهو.")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            Q = 720
            hmmm = HighQualityVideo()
            if search == 0:
                await huehue.edit(
                    "**مشش لاقي حاجه بالاسم دا اكتب حاجه عدله يعم**"
                )
            else:
                songname = search[0]
                url = search[1]
                ...
                if hm == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        await m.reply_photo(
                            photo=f"{IMAGE_THUMBNAIL}",
                            caption=f"""
**⌔ ضيفتهالك للانتظار ابسط يعم {pos}
⌔ الاسم : [{songname}]({url})
⌔ ايدي المحادثه : {chat_id}
⌔ طلب الكائن دا : {m.from_user.mention}**
""",
...
async def playlist(client, m: Message):
    ...
    if chat_id in QUEUE:
        ...
        if len(chat_queue) == 1:
            ...
            QUE = f"**⌔ الاغنيه اللي هتشتغل بعدها :** \n[{chat_queue[0][0]}]({chat_queue[0][2]}) | `{chat_queue[0][3]}` \n\n**⌔  الانتظار:**"
            l = len(chat_queue)
            for x in range(1, l):
                hmm = chat_queue[x][0]
                hmmm = chat_queue[x][2]
